...
// NEW: Handle customer creation - this is the key to automation!
async function handleCustomerCreated(customer) {
    try {
        console.log(`Processing customer creation for ${customer.id} with email ${customer.email}`);

        // Find user by email (since customer was just created)
        const userQuery = await db.collection('users').where('email', '==', customer.email).get();

        if (userQuery.empty) {
            console.log(`No user found with email: ${customer.email} - customer created before user account`);
            return;
        }
        ...
        // Link the Stripe customer ID to the Firebase user
